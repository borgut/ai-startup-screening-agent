...
async function memoStats() {
  await ensureSchema();
  const { rows } = await q(
    `SELECT COUNT(*)::int AS memos,
      COALESCE(SUM(CASE WHEN jsonb_typeof(memo_es::jsonb->'claims')='array' THEN jsonb_array_length(memo_es::jsonb->'claims') ELSE 0 END),0)::int AS claims,
      COALESCE(SUM(CASE WHEN jsonb_typeof(memo_es::jsonb->'fuentes')='array' THEN jsonb_array_length(memo_es::jsonb->'fuentes') ELSE 0 END),0)::int AS fuentes
     FROM memos`
  );
  return rows[0];
}

module.exports = { ENABLED, DIAG, ensureSchema, seedIfEmpty, listMemos, getMemo, insertMemo, getLogoBytes, saveLogoBytes, setLogoPath, memoStats };


// Lead opcional del mapa de inversores (email + filtros elegidos).
async function saveFounderInterest(email, meta) {
  await q(`CREATE TABLE IF NOT EXISTS founder_interest (
    id SERIAL PRIMARY KEY,
    email TEXT NOT NULL,
    vertical TEXT, stage TEXT, geo TEXT, raise TEXT,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now()
  )`);
  await q(
    'INSERT INTO founder_interest (email, vertical, stage, geo, raise) VALUES ($1, $2, $3, $4, $5)',
    [email, meta.vertical, meta.stage, meta.geo, meta.raise]
  );
}

module.exports.saveFounderInterest = saveFounderInterest;
