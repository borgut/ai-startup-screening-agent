...
// Algunas cadenas de Vercel Storage son de conexion directa: el pool por defecto
// las rechaza (invalid_connection_string) y pide createClient(). Probamos primero
// el pool y caemos a un cliente directo reutilizable.
let _client = null;
let _mode = null;
async function sql(strings, ...vals) {
  if (_mode === 'client') return _client.sql(strings, ...vals);
  try {
    const r = await pg.sql(strings, ...vals);
    _mode = 'pool';
    return r;
  } catch (e) {
    if (/invalid_connection_string|direct connection/i.test(String(e))) {
      _client = pg.createClient({ connectionString: process.env.POSTGRES_URL });
      await _client.connect();
      _mode = 'client';
      return _client.sql(strings, ...vals);
    }
    throw e;
  }
}
...
function ensureSchema() {
  ...
  if (!schemaPromise) {
    schemaPromise = (async () => {
      ...
      await sql`CREATE INDEX IF NOT EXISTS memos_created_idx ON memos (created_at DESC)`;
      return true;
    })().catch((e) => {
...
async function seedIfEmpty(staticRows) {
  if (seeded) return;
  const { rows } = await sql`SELECT COUNT(*)::int AS n FROM memos`;
...
async function getMemo(id, lang) {
  await ensureSchema();
  const { rows } = await sql`SELECT memo_es, memo_en, logo, logo_mime, logo_bytes IS NOT NULL AS has_logo_bytes FROM memos WHERE id = ${id}`;
...
async function getLogoBytes(id) {
  await ensureSchema();
  const { rows } = await sql`SELECT logo_mime, logo_bytes FROM memos WHERE id = ${id}`;
...
async function saveLogoBytes(id, mime, bytes) {
  await ensureSchema();
  await sql`UPDATE memos SET logo_mime = ${mime}, logo_bytes = ${bytes} WHERE id = ${id}`;
}
...
async function setLogoPath(id, logoPath) {
  await ensureSchema();
  await sql`UPDATE memos SET logo = ${logoPath} WHERE id = ${id}`;
}
