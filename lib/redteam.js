...
const ADVERSARIOS = {
  amigo: 'Quien hace el red team es un angel inversor amigo del founder: igual de riguroso con la sustancia, pero constructivo en el tono; la objecion_vc es exigente sin ser cruel.',
  vc: 'Quien hace el red team es un partner de VC early-stage: directo, sin rodeos, profesional.',
  serieb: 'Quien hace el red team es un partner de Serie B que suscribe caminos a exit: sin piedad con el tamano de mercado, el moat y el path to exit; la objecion_vc debe doler.',
};

function buildRedTeamPrompt({ url, websiteText, websiteOk, descripcion, sector, etapa, lang, adversario }) {
  const idioma = lang === 'en' ? 'ENGLISH' : 'SPANISH';
  const calibracion = ADVERSARIOS[adversario] || ADVERSARIOS.vc;
