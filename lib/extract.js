...
function extractLogoUrl(html, baseUrl) {
  ...
  while ((m = linkRe.exec(html)) !== null) {
    ...
    let score = sizes ? parseInt(sizes, 10) : 32;
    if (/apple-touch-icon/i.test(tag)) score += 200;          // suele ser 180px y cuadrado
    if (/\.svg(\?|$)/i.test(href)) score += 50;              // vectorial: mejor en tarjeta
    if (/favicon\.ico/i.test(href)) score -= 20;              // suele ser 16-48px
...
async function extractWebsite(url) {
  ...
  const result = { url: normalized, ok: false, title: '', text: '', pages: [], error: null, logoUrl: null };
  try {
    const html = await fetchPage(normalized);
    result.ok = true;
    result.title = extractTitle(html);
    result.logoUrl = extractLogoUrl(html, normalized);
